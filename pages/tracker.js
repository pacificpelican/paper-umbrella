...
function createMarkup(inner) { return { __html: inner }; };
...
function rng() { return Buffer.from('zzzzzzzzzziizzzzzzzzzzzzzzzzzzzz') }

function get_bitcoin_testnet_keypair_array() {	//	deprecated
...
export default class Tracker extends Component {
	state = { cValue: '', cVal: '', sValue: '', sVal: '', publicKeyValue: '', privateKeyValue: '', publicKeys: '', privateKeys: '', pswd: '', passwd: '', walletprops: {}, lookupWalletName: '', queryWalletName: '', displayValue: {} };
	...
	constructor(props) {
		...
		this.handlePublicKeyValueChange = this.handlePublicKeyValueChange.bind(this);
		this.handlePrivateKeyValueChange = this.handlePrivateKeyValueChange.bind(this);
		this.handlePswdValueChange = this.handlePswdValueChange.bind(this);
		this.handleWalletTitleValueChange = this.handleWalletTitleValueChange.bind(this);
	...
	handleSubmit(event) {
		...
		let theRequest = "/api/create/v/1/seed/" + encodedSeed + "/name/" + this.state.cVal + "/public/" + encodePublicKeys + "/private/" + encodedPrivateKeys + "/pswd/" + this.state.passwd;
	...
	render() {
		...
		for (let i = 0; i < iterArr.length; i++) {
			...
			dispData = dispData + "<button className='btn-tr' onClick={document.getElementById('nameinput').value=event.target.textContent;}>" + iterArr[i].name + "</button>";
		}
		...
		return (
			<div className='cryptoTracker' id={'cryptoTracker' + math_floor_random_number(500)}>
				<div className='cryptoTrackerTitleDiv'>
					<span className='cryptoWalletVal' id={'cryptoWalletVal' + math_floor_random_number(500)}><a href="http://paperumbrella.pacificio.com">☂</a> Paper Umbrella Crypto Asset Archiver</span><br />
				</div>
				<div id="walletslist">
				...
				<div id="storeddata">
					...
					<div id="adjustQuery">
						<form id="setWalletNameForLookup" onSubmit={this.handleSubmitAdjustQuery}>
							<label className="getRecord">
								Your Umbrella Password:
          						<input type="text" value={this.state.passwd} onChange={this.handlePswdValueChange} />
							</label>
							<br /><br />
							<label className="getRecord">
								Wallet Title:
				...
				<div className='cryptoInfoDisplay'>
					<section className='cryptoInfoDisplay--title'>
						<h6 id="sysPW">your Paper Umbrella system password: {this.state.passwd}</h6>
						<h2 id="cryptoLabel">Bitcoin <a href="https://electrum.org/#home">Electrum</a> Wallet Backup 🚛</h2>
						...
						<section id="pk-h1">private key(s): {this.state.privateKeys}</section>
						<form id="walletItemSave" onSubmit={this.handleSubmit}>
							...
							<label className="addRecordInput">
								Title:
          						<input type="text" value={this.state.cValue} onChange={this.handlecValueChange} />
							</label>
							<br /><br />
							<label className="addRecordInput">
								Seed*:
          						<input id="sed" type="text" value={this.state.sValue} onChange={this.handlesValueChange} />
							</label>
							<br /><br />
							<label className="addRecordInput">
								Public Key(s):
          						<input type="text" value={this.state.publicKeyValue} onChange={this.handlePublicKeyValueChange} />
							</label>
							<br /><br />
							<label className="addRecordInput">
								Private Key(s):
          						<textarea id="pks" type="text" onChange={this.handlePrivateKeyValueChange} value={this.state.privateKeyValue}>
								</textarea>
							</label>
							<br /><br />
	  ...
	  * { 
      ...
      div.cryptoTracker {
		  width: var(--ui-width);
		  background-color: var(--green-scheme-0);
		  padding-left: 0.4em;
		  padding-right: 0.3em;
	  }
	  ...
	  h6#sysPW {
		  background-color: var(--green-scheme-4);;
	  }
